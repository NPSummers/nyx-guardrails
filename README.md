...
![Nyx Guardrails demo](assets/nyx-guardrails-preview.gif)
...
```
Your tool → nyx-guardrails (detect → replace with fakes) → Provider API
Provider API → nyx-guardrails (detect fakes → restore originals) → Your tool
```
...
```bash
brew install chandika/tap/nyx-guardrails    # macOS / Linux
```

```bash
scoop bucket add chandika https://github.com/chandika/scoop-bucket
scoop install nyx-guardrails               # Windows
```

```bash
cargo install --locked --git https://github.com/chandika/nyx-guardrails  # from source
```
...
```bash
nyx-guardrails --setup
```

This scans your PATH for supported tools, installs per-tool wrappers in `~/.nyx-guardrails/bin/`, and starts the daemon as a background service (launchd on macOS, systemd on Linux, Task Scheduler on Windows).

Then add the wrapper directory to your PATH once:

```bash
export PATH="$HOME/.nyx-guardrails/bin:$PATH"
...
```bash
codex          # → filtered through nyx-guardrails
codex-direct   # → bypasses nyx-guardrails (original binary)
```
...
|---|---|
| **codex** | `~/.nyx-guardrails/bin/codex` |
| **claude** | `~/.nyx-guardrails/bin/claude` |
| **cursor** | `~/.nyx-guardrails/bin/cursor` |
| **aider** | `~/.nyx-guardrails/bin/aider` |
| **opencode** | `~/.nyx-guardrails/bin/opencode` |
...
```bash
clawdhub install nyx-guardrails
```

Registers `nyx-anthropic` as a provider. Switch to a filtered model with `/model nyx-sonnet` (or `nyx-haiku`, `nyx-opus`). All traffic through that session is filtered — no wrapper needed.
...
```bash
nyx status   # daemon running? filter active?
nyx logs     # live tail of redactions
```
...
- **Dry-run mode.** Log what would be filtered without modifying traffic: `nyx-guardrails --dry-run`
- **Encrypted vault.** Persist fake↔original mappings across restarts with AES-256-GCM + Argon2id key derivation: `NYX_GUARDRAILS_VAULT_KEY="passphrase" nyx-guardrails --setup`

---

## Comparison

| | nyx-guardrails | PasteGuard | LLM Guard | LiteLLM+Presidio |
|---|---|---|---|---|
...
Zero config needed. For fine-tuning, create `~/.config/nyx-guardrails/nyx-guardrails.yaml`:
...
nyx-guardrails [OPTIONS]
  ...
  -c, --config <PATH>         Config file path
      ...
      --vault-key <PHRASE>    Vault passphrase (or NYX_GUARDRAILS_VAULT_KEY env)
...
```bash
nyx status   # daemon running? filter on?
nyx logs     # live tail of detections
nyx on       # route this terminal through nyx-guardrails
nyx off      # this terminal goes direct (daemon keeps running)
```
