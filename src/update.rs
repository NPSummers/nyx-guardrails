...
const LATEST_RELEASE_API: &str =
    "https://api.github.com/repos/chandika/nyx-guardrails/releases/latest";
...
async fn fetch_latest_release(timeout_ms: u64, current: &str) -> Option<CachedLatestRelease> {
    ...
    let release = client
        ...
        .header(USER_AGENT, format!("nyx-guardrails/{}", current))
    ...
    let release_url = release.html_url.unwrap_or_else(|| {
        format!(
            "https://github.com/chandika/nyx-guardrails/releases/tag/v{}",
            latest
        )
    });
