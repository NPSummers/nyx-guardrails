...
struct Args {
    ...
    /// Vault encryption key (passphrase). Can also use NYX_GUARDRAILS_VAULT_KEY env var.
    #[arg(long)]
    vault_key: Option<String>,

    /// Vault file path
    #[arg(long, default_value = "./nyx-guardrails-vault.enc")]
    vault_path: String,
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let vault_key = args
        .vault_key
        .or_else(|| std::env::var("NYX_GUARDRAILS_VAULT_KEY").ok());
    ...
    let state = Arc::new(ProxyState {
        ...
        tainted_sessions: Mutex::new(HashSet::new()),
        plugin_state: Mutex::new(PluginState::new(
    ...
    if cfg.update_check.enabled && !disable_update_check_from_env() {
        ...
        tokio::spawn(async move {
            if let Some(update) = update::check_for_update(timeout_ms).await {
                ...
                eprintln!("           brew update && brew upgrade nyx-guardrails");
...
fn disable_update_check_from_env() -> bool {
    match std::env::var("NYX_GUARDRAILS_NO_UPDATE_CHECK") {
        Ok(v) => {
...
fn nyx_guardrails_dir() -> std::path::PathBuf {
    dirs_next::home_dir()
...
// wrapper scripts in ~/.nyx-guardrails/bin/ that set only the needed env vars
// and exec the real binary. Users prepend ~/.nyx-guardrails/bin to PATH.
...
fn setup_interactive(port: u16, assume_yes: bool, dry_run: bool, sensitivity: Option<&str>) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    use std::io::{IsTerminal, Write};
    let bin_dir = nyx_guardrails_dir().join("bin");

    eprintln!();
    eprintln!("  \x1b[1mnyx-guardrails — setup\x1b[0m");
    eprintln!("  ─────────────────────────────────────");
    ...
    let nyx_guardrails_home = nyx_guardrails_dir();
    std::fs::create_dir_all(&nyx_guardrails_home)?;
    ...
    install_daemon(&exe_str, port, &extra_args, &nyx_guardrails_home)?;
    ...
    if cfg!(windows) {
        eprintln!("    $env:PATH = \"$HOME\\.nyx-guardrails\\bin;\" + $env:PATH");
    } else {
        eprintln!("    export PATH=\"$HOME/.nyx-guardrails/bin:$PATH\"");
        eprintln!("    # Add to ~/.zshrc or ~/.bashrc to persist");
    }
    eprintln!();
    for name in &installed_names {
        eprintln!("    {}           → filtered through nyx-guardrails", name);
        eprintln!("    {}-direct    → bypasses nyx-guardrails completely", name);
    }
    eprintln!();
    eprintln!("  The daemon runs automatically in the background.");
    eprintln!("  Wrappers control which tools route through it.");
    eprintln!();
    eprintln!("  To uninstall everything:  nyx-guardrails --uninstall");
    eprintln!();
    Ok(())
}
...
fn uninstall_all() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    eprintln!();
    eprintln!("  \x1b[1mnyx-guardrails — uninstall\x1b[0m");
    eprintln!("  ─────────────────────────────────────");
    ...
    if std::net::TcpStream::connect("127.0.0.1:8686").is_ok() {
        ...
        #[cfg(unix)] { let _ = std::process::Command::new("pkill").args(["-f", "nyx-guardrails"]).output(); }
        #[cfg(windows)] { let _ = std::process::Command::new("taskkill").args(["/IM", "nyx-guardrails.exe", "/F"]).output(); }
        eprintln!("  ✓ Daemon stopped");
    }

    eprintln!();
    eprintln!("  Done. If you added ~/.nyx-guardrails/bin to PATH, remove that line from your shell config.");
    eprintln!();
...
fn generate_direct_wrapper(name: &str) -> String {
    if cfg!(windows) {
        format!("@echo off\r\nrem {n}-direct: bypass nyx-guardrails\r\nfor /f \"tokens=*\" %%i in ('where {n} 2^>nul') do (\r\n    echo %%i | findstr /i /c:\".nyx-guardrails\\bin\" >nul || ( %%i %* & exit /b %errorlevel% )\r\n)\r\necho could not find real '{n}' 1>&2\r\nexit /b 127\r\n", n = name)
    } else {
        format!(r#"#!/bin/sh
# {n}-direct: run {n} without nyx-guardrails
...
fn wrapper_install(port: u16) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    eprintln!();
    eprintln!("  \x1b[1mnyx-guardrails\x1b[0m — installing wrappers + daemon");
    eprintln!("  ─────────────────────────────────────");
    ...
    let nyx_guardrails_home = nyx_guardrails_dir();
    std::fs::create_dir_all(&nyx_guardrails_home)?;
    install_daemon(&exe_str, port, &[], &nyx_guardrails_home)?;
    ...
    eprintln!("    claude          → filtered through nyx-guardrails");
    eprintln!("    claude-direct   → bypasses nyx-guardrails");
    eprintln!();
...
fn generate_unix_wrapper(tool: &WrapperTool, port: u16) -> String {
    let mut script = format!(
        r#"#!/bin/sh
# nyx-guardrails wrapper for {name} — routes LLM traffic through nyx-guardrails
# Generated by: nyx-guardrails --wrapper-install
# Remove with:  nyx-guardrails --wrapper-uninstall
set -e
NYX_GUARDRAILS_PORT="${{NYX_GUARDRAILS_PORT:-{port}}}"
"#,
        name = tool.name,
        port = port,
    );
    ...
    for (var, suffix) in tool.env_vars {
        script.push_str(&format!(
            "export {var}=\"http://127.0.0.1:${{NYX_GUARDRAILS_PORT}}{suffix}\"\n",
            var = var,
            suffix = suffix,
        ));
    }
...
if [ -z "$REAL" ]; then
  echo "nyx-guardrails: could not find real '{name}' in PATH (looked everywhere except $WRAPPER_DIR)" >&2
  exit 127
fi
...
fn generate_windows_wrapper(tool: &WrapperTool, port: u16) -> String {
    let mut script = format!(
        "@echo off\r\nrem nyx-guardrails wrapper for {name}\r\nrem Generated by: nyx-guardrails --wrapper-install\r\n",
        name = tool.name,
    );
...
for /f "tokens=*" %%i in ('where {name} 2^>nul') do (
    echo %%i | findstr /i /c:".nyx-guardrails\bin" >nul || (
        %%i %*
        exit /b %errorlevel%
    )
)
echo nyx-guardrails: could not find real '{name}' in PATH 1>&2
exit /b 127
"#,
        name = tool.name,
    ));
...
fn service_install(args: &Args) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    eprintln!();
    eprintln!("  \x1b[1mnyx-guardrails\x1b[0m — installing service");
    eprintln!("  ─────────────────────────────────────");

    let shell_targets = shell_install_targets();
    let mut effective_dry_run = args.dry_run;
    match confirm_shell_integration(&shell_targets, &nyx_guardrails_home, args.yes)? {
    ...
    // Shell integration (env vars + nyx function + startup message)
    let shell_changes = install_shell(port, &nyx_guardrails_home, &shell_targets)?;

    eprintln!();
    eprintln!("  🛡️  nyx-guardrails installed and running on :{}", port);
    eprintln!();
    eprintln!("  Every new terminal will route LLM traffic through nyx-guardrails.");
    eprintln!("  To turn it off in a terminal:  nyx off");
    eprintln!("  To check status:               nyx status");
    if effective_dry_run {
        eprintln!();
    ...
    if !shell_changes.is_empty() {
        ...
        eprintln!("    nyx-guardrails --service-uninstall");
        eprintln!("    or restore backups from {}", nyx_guardrails_home.join("backups").display());
    }
...
fn service_uninstall() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    eprintln!();
    eprintln!("  Removing nyx-guardrails...");
...
fn service_status() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let daemon_ver = {
        let log_path = nyx_guardrails_dir().join("nyx-guardrails.log");
        if let Ok(contents) = std::fs::read_to_string(log_path) {
            contents
                .lines()
                .find(|l| l.contains("nyx-guardrails v"))
                .and_then(|l| {
                    l.split('v')
                        .nth(1)
    ...
    eprintln!(
        "  filter:   {}",
        if active {
            "✓ on (LLM traffic routed through nyx-guardrails)"
        } else {
    ...
    eprintln!();
    if running && !active {
        eprintln!("  Run `nyx on` or open a new terminal.");
    } else if !running {
        eprintln!("  Run `nyx-guardrails --service-install` to set up.");
    }
...
fn install_daemon(
    ...
    port: u16,
    extra_args: &[String],
    nyx_guardrails_home: &std::path::Path,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let plist_path = dirs_next::home_dir()
        .unwrap()
        .join("Library/LaunchAgents/com.nyx-guardrails.plist");
...
<dict>
    <key>Label</key>
    <string>com.nyx-guardrails</string>
...
{extra}
    ...
    <string>{home}/nyx-guardrails.log</string>
    ...
    <string>{home}</string>
        ...
        port = port,
        extra = extra_xml,
        home = nyx_guardrails_home.to_string_lossy(),
    );
...
fn install_daemon(
    ...
    let unit_path = unit_dir.join("nyx-guardrails.service");
    ...
    let unit = format!(
        r#"[Unit]
Description=nyx-guardrails — invisible secrets filter for LLM APIs
...
Restart=always
RestartSec=2
StandardOutput=append:{home}/nyx-guardrails.log
StandardError=append:{home}/nyx-guardrails.log
...
"#,
    ...
    let status = std::process::Command::new("systemctl")
        .args(["--user", "enable", "--now", "nyx-guardrails"])
        .status()?;
...
fn install_daemon(
    ...
    let task_xml_path = nyx_guardrails_home.join("nyx-guardrails-task.xml");
    let task_xml = format!(
        r#"<?xml version="1.0" encoding="UTF-16"?>
<Task version="1.4" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  <RegistrationInfo>
    <Description>nyx-guardrails — invisible secrets filter for LLM APIs</Description>
  </RegistrationInfo>
  <Triggers>
    <LogonTrigger>
  ...
  <Actions>
    ...
    let _ = std::process::Command::new("schtasks")
        .args(["/Delete", "/TN", "nyx-guardrails", "/F"])
        .output();
...
fn install_daemon(
    ...
    _port: u16,
    _extra_args: &[String],
    _nyx_guardrails_home: &std::path::Path,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    eprintln!("  ⚠ Unsupported platform for service install.");
    eprintln!("  Run `nyx-guardrails` manually in the background.");
    Ok(())
}
...
fn uninstall_daemon() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let _ = std::process::Command::new("systemctl")
        .args(["--user", "disable", "--now", "nyx-guardrails"])
        .status();
    let unit_path = dirs_next::home_dir()
        .unwrap()
        .join(".config/systemd/user/nyx-guardrails.service");
    if unit_path.exists() {
...
const SHELL_MARKER_START: &str = "# >>> nyx-guardrails >>>";
const SHELL_MARKER_END: &str = "# <<< nyx-guardrails <<<";

const BASH_ZSH_BLOCK: &str = r#"
# Env vars — route LLM traffic through nyx-guardrails (default: on)
...
_nyx_guardrails_version() {
  command nyx-guardrails --version 2>/dev/null | awk '{print $2}'
}

# Startup message
if [ -z "${NYX_GUARDRAILS_QUIET:-}" ]; then
  if curl -so /dev/null -w '%{http_code}' "http://127.0.0.1:8686/" 2>/dev/null | grep -qE '^(200|404|502)$'; then
    echo "🛡️ nyx-guardrails active (v$(_nyx_guardrails_version))"
  fi
fi

# Toggle function
nyx() {
  local port="${NYX_GUARDRAILS_PORT:-8686}"
  local base="http://127.0.0.1:${port}"
  local ver="$(_nyx_guardrails_version)"
  case "${1:-status}" in
    on)
      if ! curl -so /dev/null -w '%{http_code}' "${base}/" 2>/dev/null | grep -qE '^(200|404|502)$'; then
        echo "  ✗ nyx-guardrails daemon not running on :${port}"
        echo "  Run: nyx-guardrails --service-install"
        return 1
      fi
      ...
      echo "  🛡️ nyx-guardrails on (v${ver}) — LLM traffic filtered"
      ;;
    off)
      ...
      echo "  nyx-guardrails off — traffic going direct"
      ;;
    logs)
      local logp="$HOME/.nyx-guardrails/nyx-guardrails.log"
      if [ ! -f "$logp" ]; then
        echo "  ✗ no log file at $logp"
        return 1
      fi
      echo "  tailing $logp (ctrl+c to stop)"
      tail -f "$logp" | grep --line-buffered -E '🛡️|⚠️|📎|📊|nyx-guardrails v'
      ;;
    status)
      ...
      if [ -f "$HOME/.nyx-guardrails/nyx-guardrails.log" ]; then
        daemon_ver=$(grep -m1 -E 'nyx-guardrails v[0-9]' "$HOME/.nyx-guardrails/nyx-guardrails.log" | sed -E 's/.*v([0-9]+\.[0-9]+\.[0-9]+).*//' )
      fi
      echo ""
      echo "  nyx-guardrails"
      echo "  ─────────────────────────────"
      ...
      ;;
    *)
      echo "Usage: nyx [on|off|status|logs]"
      ;;
  esac
}
"#;
...
function _MirageVersion {
    try { return (nyx-guardrails --version).Split(' ')[1] } catch { return "unknown" }
}

# Startup message
if (-not $env:NYX_GUARDRAILS_QUIET) {
    try {
        $resp = Invoke-WebRequest -Uri "http://127.0.0.1:8686/" -TimeoutSec 1 -UseBasicParsing -ErrorAction SilentlyContinue
        Write-Host "🛡️ nyx-guardrails active (v$(_MirageVersion))"
    } catch [System.Net.WebException] {
        if ($_.Exception.Response) { Write-Host "🛡️ nyx-guardrails active (v$(_MirageVersion))" }
    } catch {}
}
...
function nyx {
    ...
    $port = if ($env:NYX_GUARDRAILS_PORT) { $env:NYX_GUARDRAILS_PORT } else { "8686" }
    ...
    switch ($Action) {
        "on" {
            ...
            if (-not $daemon_up) {
                Write-Host "  ✗ nyx-guardrails daemon not running on :$port"
                Write-Host "  Run: nyx-guardrails --service-install"
                return
            }
            ...
            Write-Host "  🛡️ nyx-guardrails on (v$ver) — LLM traffic filtered"
        }
        "off" {
            ...
            Write-Host "  nyx-guardrails off — traffic going direct"
        }
        "logs" {
            $log = Join-Path $HOME ".nyx-guardrails/nyx-guardrails.log"
            if (-not (Test-Path $log)) { Write-Host "  ✗ no log file at $log"; return }
            Write-Host "  tailing $log (ctrl+c to stop)"
            Get-Content -Path $log -Wait | Select-String -Pattern "🛡️|⚠️|📎|📊|nyx-guardrails v"
        }
        "status" {
            $running = $false
            try {
            ...
            $log = Join-Path $HOME ".nyx-guardrails/nyx-guardrails.log"
            if (Test-Path $log) {
                $line = Select-String -Path $log -Pattern "nyx-guardrails v[0-9]" | Select-Object -First 1
                if ($line) { $daemonVer = ($line.Line -replace '.*v([0-9]+\.[0-9]+\.[0-9]+).*','$1') }
            }
            Write-Host ""
            Write-Host "  nyx-guardrails"
            Write-Host "  ─────────────────────────────"
        ...
        default {
            Write-Host "Usage: nyx [on|off|status|logs]"
        }
    }
}
"#;

const PS_MARKER_START: &str = "# >>> nyx-guardrails >>>";
const PS_MARKER_END: &str = "# <<< nyx-guardrails <<<";
...
fn confirm_shell_integration(
    ...
    let backup_dir = nyx_guardrails_home.join("backups");

    eprintln!("  This install will:");
    eprintln!("    1) Install a background service for auto-start");
    eprintln!("    2) Add a managed shell block to enable nyx-guardrails by default");
    ...
    eprintln!("  Revert options: `nyx-guardrails --service-uninstall` or restore a backup file.");
...
fn tail_log(nyx_guardrails_home: &std::path::Path) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let log_path = nyx_guardrails_home.join("nyx-guardrails.log");
