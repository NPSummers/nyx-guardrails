...
const PLUGIN_ID = "nyx-guardrails";
const PLUGIN_NAME = "nyx-guardrails";
...
const openClawGuardPlugin = {
  id: PLUGIN_ID,
  name: PLUGIN_NAME,
  description: "Nyx Guardrails plugin for OpenClaw",

  register(api: OpenClawPluginApi) {
    const log = createLogger(api.logger);

    // ── Initialize Nyx routing manager ────────────────────────
    // Users enable sanitization via /nyx_sanitize on, which routes agents through Nyx.
    // Async: waits for port availability (old process may hold it during plugin update).
    startGateway()
      .then(() => log.debug?.("Nyx routing manager initialized"))
      .catch((err) => log.error(`Failed to initialize Nyx routing manager: ${err}`));
    ...
    // Capture session start metadata (without prompt interception/content forwarding)
    api.on("before_agent_start", async (event, ctx) => {
      ...
      globalEventReporter?.report(sessionKey, "before_agent_start", {
        ...
        prompt: "[omitted by nyx-guardrails plugin]",
    ...
    // Prompt interception hooks are intentionally disabled.
    // Nyx performs request/response guardrails at the proxy layer.
    ...
    // message_sending interception removed intentionally.
    ...
    api.registerCommand({
      ...
      handler: async () => {
        ...
        const lines = [
          ...
          "Commands:",
          ...(isEnterprise ? [] : [
            "- /nyx_core — Open Core portal to upgrade plan",
            "- /nyx_claim — Show agent info for claiming",
          ]),
          "- /nyx_config — Configure API key",
        ];
    ...
    api.registerCommand({
      ...
      handler: async () => {
        ...
        return {
          text: [
            "**Configure nyx-guardrails API Key**",
            "",
    ...
    api.registerCommand({
      ...
      handler: async () => {
        if (!globalCoreCredentials) {
          return {
            text: "nyx-guardrails not registered yet. It will auto-register on first use.",
          };
        }
      ...
      handler: async (ctx) => {
        ...
        if (command === "on") {
          ...
          try {
            ...
            return {
              text: [
                "**Nyx Routing Enabled**",
                "",
                "All LLM requests will now be routed through your local Nyx API.",
                "Nyx performs request/response sanitization at the proxy layer.",
                "",
                `- Nyx URL: ${getGatewayStatus().url}`,
                `- Providers protected: ${result.providers.join(", ")}`,
                "",
                result.warnings.length > 0 ? "**Warnings:**" : "",
                ...result.warnings.map(w => `  ${w}`),
                result.warnings.length > 0 ? "" : "",
                "**IMPORTANT:** Do not add/modify providers in openclaw.json while Nyx routing is enabled.",
                "To add/modify providers:",
                "  1. Run `/nyx_sanitize off`",
                "  2. Modify openclaw.json",
                "  3. Run `/nyx_sanitize on`",
                "",
                "Configuration modified: ~/.openclaw/openclaw.json",
                "To disable, run: `/nyx_sanitize off`",
              ].filter(Boolean).join("\n"),
            };
          } catch (err) {
            return {
              text: [
                "**Failed to Enable Nyx Routing**",
                "",
                `Error: ${err instanceof Error ? err.message : String(err)}`,
                "",
                "Nyx routing setup failed. Verify nyx-guardrails is installed and reachable.",
              ].join("\n"),
            };
          }
        } else if (command === "off") {
          ...
          try {
            ...
            if (!status.enabled) {
              return {
                text: "Nyx routing is not currently enabled.",
              };
            }
            ...
            return {
              text: [
                "**Nyx Routing Disabled**",
                "",
                "LLM requests will now go directly to providers (Nyx bypassed).",
                "",
                ...
                "Nyx process lifecycle is managed outside this plugin.",
              ].filter(Boolean).join("\n"),
            };
          } catch (err) {
            return {
              text: [
                "**Failed to Disable Nyx Routing**",
                "",
                `Error: ${err instanceof Error ? err.message : String(err)}`,
              ].join("\n"),
            };
          ...
          return {
            text: [
              ...
              "",
              "Usage:",
              "  /nyx_sanitize on  — Enable data sanitization",
              "  /nyx_sanitize off — Disable data sanitization",
              "",
              "Nyx protects sensitive data before sending to LLMs:",
        ...
        try {
          ...
          if (scanType === "summary" || scanType === "info") {
            ...
            return {
              text: [
                ...
                "",
                "Run `/nyx_scan all` to scan all files for security risks.",
              ].join("\n"),
            };
          }
          ...
          if (!globalDashboardClient) {
            try {
              ...
              const tokenFile = path.join(os.homedir(), ".openclaw", "credentials", "nyx-guardrails", "dashboard-session-token");
          ...
          lines.push("", `Full details available in dashboard: /nyx_dashboard`);
    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        if (command === "on") {
          ...
          if (!globalCoreCredentials) {
            return {
              text: "Cannot enable auto-scan: nyx-guardrails not registered yet.",
            };
          }
          ...
          return {
            text: [
              ...
              "",
              "View scan results in Dashboard: `/nyx_dashboard`",
              "",
              "To disable: `/nyx_autoscan off`",
            ].join("\n"),
          };
        } else if (command === "off") {
          if (!autoScanEnabled || !globalFileWatcher?.running) {
          ...
          return {
            text: [
              ...
              "",
              "Usage:",
              "  /nyx_autoscan on  — Enable automatic scanning",
              "  /nyx_autoscan off — Disable automatic scanning",
              "",
    ...
    api.registerCommand({
      ...
      handler: async () => {
        ...
        if (!deleted && !hadCredentials) {
          return {
            text: [
              "**nyx-guardrails Reset**",
              "",
              "No credentials to reset. nyx-guardrails will auto-register on next use.",
            ].join("\n"),
          };
        }
        ...
        try {
          ...
          return {
            text: [
              ...
              "Run `/nyx_status` to check your quota.",
            ].filter(Boolean).join("\n"),
          };
        } catch (err) {
          return {
            text: [
              ...
              `${err}`,
              "",
              "nyx-guardrails will try to register again on next use.",
            ].join("\n"),
          };
        }
      },
